...
| File | Why |
|---|---|
| `client/src/scenarios/scenarios.ts` | Scenario entry (and Domain entry if new) |
| `client/src/scenarios/steps/<domain>.ts` | Step entries for the flow |
| `client/src/scenarios/data.ts` | Barrel — only if you add a new steps file |
| `client/src/scenarios/services.ts` / `topics.ts` | Any missing services/topics |
...
1. **Domain** — which domain does this flow belong to? Check `DOMAINS` in `client/src/scenarios/scenarios.ts` (the demo ships with `shopping`, `fulfillment`, `engagement`). If it doesn't fit, confirm a new domain name — you'll add a `Domain` entry too.
...
```bash
grep -rh 'phaseId:' client/src/scenarios/scenarios.ts | grep -o '[0-9]*' | sort -n | tail -1
```
...
Append to the domain's steps file (`client/src/scenarios/steps/<domain>.ts`). All steps for this scenario share `phase: <phaseId>`.
...
npm run validate       # ids resolve, phaseIds unique, snapshot fresh
npm run build          # vite build must pass
npm run dev            # eyeball the new scenario
```

Commit the regenerated `server/src/generated/cosmos-map.json` together with the data edits — `npm run validate` fails with `cosmos-map.json is stale — run npm run snapshot` otherwise.
...
- Wrong steps playing? → Two steps with the same `phase` from different scenarios (`grep -rn "phase: <N>" client/src/scenarios/steps/`)
