...
- Stage 2: PLANNED — M0: repoint drift-sync (imports, WRITABLE_PATHS, prompts, tsconfig, README) + scripts/fresh-start.mjs to client/src
- Stage 3: PLANNED — M0: server/ workspace skeleton + snapshot-map.ts + committed cosmos-map.json + validate freshness check + apply-edits snapshot hook ⚠ generated JSON oversized, cannot split further
